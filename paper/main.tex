...
People express themselves differently across contexts. A dating profile foregrounds personality, values, and lifestyle; a resume foregrounds skills, experience, and professional demeanor. Yet both documents are authored by the same person, and aspects of how they present themselves---word choice, communication style, the way they frame priorities---leak through regardless of domain. Can we learn embeddings that capture this shared signal?
...
\begin{itemize}
  \item \textbf{Cross-domain traits}: Big Five personality scores (Gaussian, 0--100), core values, MBTI (correlated with Big Five), communication style, attachment style.
...
\paragraph{A note on identity proxies.}
We do not claim that Big Five scores or MBTI types are \emph{true} domain-invariant features of human identity. Personality psychology provides useful heuristics for generating synthetic people with internally coherent attributes, but any trait taxonomy is an imperfect approximation of the latent structure underlying human self-presentation \citep{block1995contrarian,mccrae2008big5limits}. Our experiment tests whether an encoder can learn to align \emph{whatever} shared signal exists between two domains---the model receives only rendered text, never raw trait scores. If the cross-domain contrastive loss succeeds, it is because the generated texts contain recoverable stylistic and semantic regularities, not because Big Five is ground truth.
...
\begin{itemize}
  ...
  \item \textbf{Optimizer}: AdamW, learning rate $2 \times 10^{-5}$, linear warmup + cosine decay
...
More fundamentally, our synthetic people are generated from structured trait taxonomies (Big Five, MBTI) that are themselves imperfect models of human personality. Real identity is richer, more contextual, and less neatly decomposable than any trait inventory suggests. Our experiment demonstrates a \emph{method}---prefix-targeted cross-domain contrastive alignment---not a claim about the structure of human identity. The method should generalize to any setting where the same entity produces semantically different texts across domains, regardless of the generative process.
...
\paragraph{Connection to embedding translation.}
Our work is conceptually related to the embedding model upgrade problem, where adapters translate between old and new embedding spaces \citep{vejendla2025drift}. Both settings involve learning a mapping between two embedding spaces that encode overlapping but not identical information. The key difference is that model upgrade translation operates between spaces with the same semantic function (retrieval) but different parameterizations, while our cross-domain prefix alignment operates between spaces with different semantic functions (dating vs.\ hiring) but shared underlying entities. Local drift-adapters that specialize by region \citep{dan2020cluster} are particularly relevant: our matryoshka prefix is, in effect, a ``region'' of the embedding where the cross-domain mapping is enforced to be identity.
